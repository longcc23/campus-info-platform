...

{
    ...
    "key_info": {
        ...
        "link": "投递链接/问卷链接/邮箱地址 (完整的URL如 https://career.wjx.cn/vm/eCMU7Q0.aspx，或邮箱地址如 cdcresume@sem.tsinghua.edu.cn，如果没有则为空字符串)",
...
4. **链接信息**：必须提取所有URL链接（问卷链接、投递链接等）。**特别注意**：如果是邮箱地址（如 xxx@xxx.com），也要提取到 link 字段中。
5. **邮箱和投递信息（非常重要）**：
...
9. **时间信息提取（重要）**：
   ...
   - date：如果是活动/讲座，提取活动日期（格式如"12月4日"或"2025年12月4日"）。**注意**：

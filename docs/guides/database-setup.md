# 📊 数据库初始化指南

本指南说明如何在 Supabase 中初始化数据库表。

---

## 📋 执行顺序

### 第一步：创建主表（Events）

1. 打开 Supabase 控制台
2. 进入 **SQL Editor**
3. 复制 `supabase_schema.sql` 的全部内容
4. 粘贴到 SQL Editor
5. 点击 **Run** 执行

**验证**：
```sql
SELECT * FROM events LIMIT 1;
```

### 第二步：创建用户相关表（Users, Favorites, View History）

1. 在同一个 SQL Editor 中
2. 复制 `supabase_schema_users.sql` 的全部内容
3. 粘贴到 SQL Editor
4. 点击 **Run** 执行

**验证**：
```sql
-- 检查表是否创建成功
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('users', 'favorites', 'view_history');

-- 应该返回 3 行
```

---

## 🔍 执行后的验证

### 检查所有表

```sql
SELECT 
    table_name,
    (SELECT COUNT(*) FROM information_schema.columns 
     WHERE table_name = t.table_name) as column_count
FROM information_schema.tables t
WHERE table_schema = 'public'
AND table_type = 'BASE TABLE'
ORDER BY table_name;
```

**预期结果**：
- `events` - 主表
- `favorites` - 收藏表
- `users` - 用户表
- `view_history` - 浏览足迹表

### 检查 RLS 策略

```sql
SELECT 
    schemaname,
    tablename,
    policyname,
    permissive,
    roles,
    cmd
FROM pg_policies
WHERE schemaname = 'public'
ORDER BY tablename, policyname;
```

### 检查索引

```sql
SELECT 
    tablename,
    indexname,
    indexdef
FROM pg_indexes
WHERE schemaname = 'public'
ORDER BY tablename, indexname;
```

---

## ⚠️ 常见问题

### 1. 外键约束错误

**错误信息**：`relation "events" does not exist`

**原因**：先执行了 `supabase_schema_users.sql`，但 `events` 表还未创建

**解决**：先执行 `supabase_schema.sql`，再执行 `supabase_schema_users.sql`

### 2. 策略已存在错误

**错误信息**：`policy "Allow public read access on users" already exists`

**原因**：重复执行了 SQL 脚本

**解决**：可以忽略，或先删除策略再执行：
```sql
DROP POLICY IF EXISTS "Allow public read access on users" ON users;
```

### 3. 触发器函数已存在

**错误信息**：`function cleanup_old_view_history() already exists`

**解决**：脚本中已使用 `CREATE OR REPLACE FUNCTION`，可以安全重复执行

---

## 📝 快速执行脚本

如果遇到问题，可以分步执行：

### 步骤 1：创建 Users 表

```sql
CREATE TABLE IF NOT EXISTS users (
    openid TEXT PRIMARY KEY,
    last_seen TIMESTAMPTZ DEFAULT NOW(),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_users_last_seen ON users(last_seen DESC);

ALTER TABLE users ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public read access on users"
    ON users FOR SELECT TO public USING (true);

CREATE POLICY "Allow public insert users"
    ON users FOR INSERT TO public WITH CHECK (true);

CREATE POLICY "Allow users to update own record"
    ON users FOR UPDATE TO public USING (true) WITH CHECK (true);
```

### 步骤 2：创建 Favorites 表

```sql
CREATE TABLE IF NOT EXISTS favorites (
    id BIGSERIAL PRIMARY KEY,
    user_id TEXT NOT NULL REFERENCES users(openid) ON DELETE CASCADE,
    event_id BIGINT NOT NULL REFERENCES events(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(user_id, event_id)
);

CREATE INDEX IF NOT EXISTS idx_favorites_user_id ON favorites(user_id);
CREATE INDEX IF NOT EXISTS idx_favorites_event_id ON favorites(event_id);
CREATE INDEX IF NOT EXISTS idx_favorites_created_at ON favorites(created_at DESC);

ALTER TABLE favorites ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public read access on favorites"
    ON favorites FOR SELECT TO public USING (true);

CREATE POLICY "Allow public insert favorites"
    ON favorites FOR INSERT TO public WITH CHECK (true);

CREATE POLICY "Allow public delete favorites"
    ON favorites FOR DELETE TO public USING (true);
```

### 步骤 3：创建 View History 表

```sql
CREATE TABLE IF NOT EXISTS view_history (
    id BIGSERIAL PRIMARY KEY,
    user_id TEXT NOT NULL REFERENCES users(openid) ON DELETE CASCADE,
    event_id BIGINT NOT NULL REFERENCES events(id) ON DELETE CASCADE,
    viewed_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_view_history_user_id ON view_history(user_id);
CREATE INDEX IF NOT EXISTS idx_view_history_viewed_at ON view_history(viewed_at DESC);

ALTER TABLE view_history ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public read own view history"
    ON view_history FOR SELECT TO public USING (true);

CREATE POLICY "Allow public insert view history"
    ON view_history FOR INSERT TO public WITH CHECK (true);
```

### 步骤 4：创建自动清理触发器

```sql
CREATE OR REPLACE FUNCTION cleanup_old_view_history()
RETURNS TRIGGER AS $$
BEGIN
    DELETE FROM view_history
    WHERE user_id = NEW.user_id
    AND id NOT IN (
        SELECT id FROM view_history
        WHERE user_id = NEW.user_id
        ORDER BY viewed_at DESC
        LIMIT 20
    );
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER cleanup_view_history_trigger
    AFTER INSERT ON view_history
    FOR EACH ROW
    EXECUTE FUNCTION cleanup_old_view_history();
```

---

## ✅ 执行成功标志

执行成功后，您应该看到：

1. **表创建成功**：4 张表（events, users, favorites, view_history）
2. **索引创建成功**：每张表都有相应的索引
3. **RLS 策略创建成功**：每张表都有相应的安全策略
4. **触发器创建成功**：view_history 表有自动清理触发器

---

## 🔗 相关文档

- [Supabase集成指南.md](./Supabase集成指南.md)
- [Supabase连接测试指南.md](./Supabase连接测试指南.md)

---

**最后更新**：2025年12月4日








